...
(defmacro* psql:define-parameter (name default-value type docstring)
  ;; TODO: Gather all the information for help.
  `(progn
     (defvar ,name ,default-value ,docstring)
     (make-variable-buffer-local ',name)
     ',name))

(defmacro* psql:define-command (name parameters docstring &body body)
  `(progn
...
;; (defun psql:pset (option value)
;;   )
;; (defun psql:pget (option)
;;   )
;; (defsetf psql:pget (option) (value) `(psql:pset ,option ,value))

(psql:define-parameter psql:*print-aligned* t
  boolean "Whether the fields must be aligned in columns.")

  
(psql:define-command a ()
  "Toggle field alignment This makes little sense but we keep it around."
 (setf  psql:*print-aligned* (not psql:*print-aligned*)))


(psql:define-command C (title)
  "Override table title (formerly change HTML caption)."
  (setf psql:*table-title* title))


(psql:define-command c (dbname user host port)
  "
...
"
  ;; TODO: implement getting the default values, and the coding.
  (psql dbname user (read-passwd "Password: ") host port coding))


(psql:define-command cd (path)
  "Change the current directory."
  ;; TODO: Check the path.
  (setf default-directory path))

(psql:define-command copy ()
  "copy"
;; 	/* \copy */
...
;;  *
;;  * $PostgreSQL: pgsql/src/bin/psql/describe.c,v 1.218.2.3 2010/03/01 20:55:53 heikki Exp $
;;  */
